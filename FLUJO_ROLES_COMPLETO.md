...
### 👮 Flujo del STAFF (Personal de Parada)

**Objetivo**: Validar pasajeros en la parada asignada usando escaneo de QR.

#### **DÍA DEL VIAJE - Flujo en Parada**

```
PREPARACIÓN:
1️⃣ Consultar mis viajes asignados
   GET /api/Viajes/mis-viajes
   Response: [
     {
       "viajeID": 1,
       "paradaAsignada": "Centro Guadalajara",
       "horaEstimadaLlegada": "14:00:00",
       "pasajerosEsperados": 12
     }
   ]

CUANDO LLEGA EL CHOFER:
2️⃣ Recibo notificación automática
   📱 "El chofer del viaje VJE-2025122500001 ha llegado a Centro Guadalajara"
   
3️⃣ Iniciar proceso de validación
   POST /api/viajes/1/checkin/iniciar-validacion
   Body: {
     "paradaViajeID": 1
   }
   
   ✅ Estado de parada cambia: "Llegado" → "Validando"

VALIDAR PASAJEROS:
4️⃣ Escanear QR de cada pasajero
   POST /api/Boletos/5/validar
   Body: {
     "latitud": 20.676682,
     "longitud": -103.346677
   }
   ...
   ❌ Si hay error: {
     ...
     "fechaValidacion": "2025-12-25T13:58:00"
   }
   
   Repetir para cada pasajero (12 veces en este ejemplo)

FINALIZAR:
5️⃣ Todos abordados → Finalizar validación
   POST /api/viajes/1/checkin/finalizar-validacion
   Body: {
     "paradaViajeID": 1,
     "totalAbordados": 11,
     "totalNoShow": 1,
     "observaciones": "Juan Pérez no se presentó"
   }
   
   ✅ Sistema automáticamente:
      - Marca parada como "Completado"
      - Actualiza contadores del viaje
      - Notifica al chofer: "Puede continuar a siguiente parada"
      - Registra estadísticas en el manifiesto

SI HAY PROBLEMAS:
6️⃣ Reportar incidencia
   POST /api/Incidencias
   Body: {
     "viajeID": 1,
     "paradaViajeID": 1,
     "tipoIncidenciaID": 3,
     "descripcion": "Pasajero sin identificación",
     "severidad": "Baja"
   }
```
