...
public class RutasController : ControllerBase
{
    ...
    [HttpPut("{id}")]
    [ClRequirePermission(ClAppPermissions.RutasUpdate)]
    public async Task<ActionResult<PlantillaRutaResponseDto>> UpdateRuta(int id, [FromBody] ActualizarPlantillaRutaDto dto)
    {
        try
        {
            var ruta = await _context.PlantillasRutas
            ...
            // Validar que no exista otra ruta con el mismo código (excluyendo la actual)
            var existeCodigo = await _context.PlantillasRutas
                .AnyAsync(r => r.CodigoRuta == dto.CodigoRuta && r.RutaID != id);

            if (existeCodigo)
                return BadRequest(new { message = "Ya existe otra ruta con ese código" });
            ...
            ruta.CodigoRuta = dto.CodigoRuta;
            ruta.NombreRuta = dto.NombreRuta;
            ruta.CiudadOrigen = dto.CiudadOrigen;
            ruta.CiudadDestino = dto.CiudadDestino;
            ruta.PuntoPartidaLat = dto.PuntoPartidaLat;
            ruta.PuntoPartidaLong = dto.PuntoPartidaLong;
            ruta.PuntoPartidaNombre = dto.PuntoPartidaNombre;
            ruta.PuntoLlegadaLat = dto.PuntoLlegadaLat;
            ruta.PuntoLlegadaLong = dto.PuntoLlegadaLong;
            ruta.PuntoLlegadaNombre = dto.PuntoLlegadaNombre;
            ruta.DistanciaKm = dto.DistanciaKm;
            ruta.TiempoEstimadoMinutos = dto.TiempoEstimadoMinutos;

            // Actualizar paradas
            if (dto.Paradas != null)
            {
                // 1. Identificar paradas a mantener/actualizar y nuevas
                var paradasDtoIds = dto.Paradas
                    .Where(p => p.ParadaID.HasValue)
                    .Select(p => p.ParadaID.Value)
                    .ToList();

                // 2. Soft delete para paradas que ya no están en el DTO
                // Solo marcamos como inactivas las que estaban activas y ya no vienen
                foreach (var paradaExistente in ruta.Paradas.Where(p => p.EsActiva))
                {
                    if (!paradasDtoIds.Contains(paradaExistente.ParadaID))
                    {
                        paradaExistente.EsActiva = false;
                    }
                }

                // 3. Actualizar existentes y Agregar nuevas
                foreach (var paradaDto in dto.Paradas)
                {
                    if (paradaDto.ParadaID.HasValue)
                    {
                        // Actualizar existente
                        var paradaExistente = ruta.Paradas.FirstOrDefault(p => p.ParadaID == paradaDto.ParadaID.Value);
                        if (paradaExistente != null)
                        {
                            paradaExistente.NombreParada = paradaDto.NombreParada;
                            paradaExistente.Latitud = paradaDto.Latitud;
                            paradaExistente.Longitud = paradaDto.Longitud;
                            paradaExistente.Direccion = paradaDto.Direccion;
                            paradaExistente.OrdenParada = paradaDto.OrdenParada;
                            paradaExistente.TiempoEsperaMinutos = paradaDto.TiempoEsperaMinutos;
                            paradaExistente.EsActiva = true; // Asegurar que esté activa si se envía
                        }
                        // Si envían un ID que no pertenece a esta ruta, lo ignoramos o podríamos lanzar error.
                        // Por seguridad, solo actualizamos si está en la colección de la ruta.
                    }
                    else
                    {
                        // Agregar nueva
                        var nuevaParada = new PlantillaParada
                        {
                            PlantillaRutaID = ruta.RutaID,
                            NombreParada = paradaDto.NombreParada,
                            Latitud = paradaDto.Latitud,
                        ...
                        _context.PlantillasParadas.Add(nuevaParada);
                    }
                }
            }
            ...
            var response = await _context.PlantillasRutas
                ...
                .Select(r => new PlantillaRutaResponseDto
                {
                    ...
                    Activa = r.Activa,
                    FechaCreacion = r.FechaCreacion,
                    TotalParadas = r.Paradas.Count(p => p.EsActiva), // Solo contar activas
                    TotalViajes = r.Viajes.Count
                })
                .FirstOrDefaultAsync();

            _logger.LogInformation("Ruta {RutaID} actualizada", id);

            return Ok(response);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error al actualizar ruta {RutaID}", id);
            return StatusCode(500, new { message = "Error al actualizar ruta", error = ex.Message });
        }
    }
}
